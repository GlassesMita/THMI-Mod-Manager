...
class TestSilentElevation
{
    static void Main()
    {
        ...
        Logger.LogInfo($"Is administrator: {isAdmin}");
        
        // 如果当前不是High完整性级别，尝试静默提升
        if (currentLevel != "High")
        {
            Console.WriteLine("\n尝试静默提升权限...");
            Logger.LogInfo("\nAttempting silent privilege elevation...");
            
            // 测试CMSTP方法
            Console.WriteLine("测试CMSTP白名单方法...");
            Logger.LogInfo("Testing CMSTP whitelist method...");
            bool cmstpResult = SilentPermissionHelper.ElevateUsingWhiteList();
            Console.WriteLine($"CMSTP方法结果: {cmstpResult}");
            Logger.LogInfo($"CMSTP method result: {cmstpResult}");
            
            if (!cmstpResult)
            {
                // 测试计划任务方法
                Console.WriteLine("测试计划任务方法...");
                Logger.LogInfo("Testing scheduled task method...");
            ...
            Logger.LogInfo($"Elevated integrity level: {newLevel}");
            
            if (newLevel == "High")
            {
                Console.WriteLine("静默提升成功！配置特权...");
                Logger.LogInfo("Silent elevation successful! Configuring privileges...");
                bool configResult = SilentPermissionHelper.ConfigurePrivileges();
                Console.WriteLine($"特权配置结果: {configResult}");
                Logger.LogInfo($"Privilege configuration result: {configResult}");
                
                // 再次检查特权状态
                string newPrivilegeStatus = SilentPermissionHelper.GetPrivilegeStatus();
                Console.WriteLine($"配置后特权状态:\n{newPrivilegeStatus}");
                Logger.LogInfo($"Privilege status after configuration:\n{newPrivilegeStatus}");
                Logger.LogInfo($"Privilege status after configuration:\n{newPrivilegeStatus}");
            }
        }
        else
        {
            Console.WriteLine("\n已具有High完整性级别，配置特权...");
            Logger.LogInfo("\nAlready have High integrity level, configuring privileges...");
            bool configResult = SilentPermissionHelper.ConfigurePrivileges();
            Console.WriteLine($"特权配置结果: {configResult}");
